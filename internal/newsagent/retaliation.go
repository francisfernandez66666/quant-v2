...
// English: package newsagent derives policy-retaliation events. It detects keywords like retaliation / export control / rare-earth limits / tariff retaliation from foreign-policy headlines and directly builds data.ConfrontationEvent without the LLM, for stable recognition of foreign-policy news.
package newsagent
...
// English: retaliationRule is a keyword-matching rule; a matched headline yields a ConfrontationEvent for the mapped sectors/direction.
type retaliationRule struct {
	keywords []string // 触发关键词（任一命中即可）
	// English: trigger keywords (any single hit is enough)
	sectors []string // 受影响板块
	// English: affected sectors
	direction string // 方向：利好/利空
	// English: direction: bullish / bearish
	impact string // 影响级别：高/中/低
	// English: impact level: high / medium / low
}
...
// English: retaliationRules covers the main A-share retaliation scenarios: tariff hikes, export controls, rare-earth/resource limits, anti-dumping, etc.
...
// English: DeriveRetaliation builds foreign-policy retaliation events from headlines; any keyword hit yields an event, nil when none.
...
// English: matchRetaliation returns the first matching retaliation rule for a title, nil when none.
